...
int main(int argc, char** argv)
{
    ...
    {
        if(algo=="sgbm"){
	    std::cout << "Algo is SGBM!" << std::endl; 
    ...
    if(filter=="wls_conf") // filtering with confidence (significantly better quality than wls_no_conf)
    {
        ...
        {
	    //cout << "Algo is here! At line 170." << endl;
	    //cout << "We are at line 171 and printing max_disp value: " << max_disp << endl;
            Ptr<StereoSGBM> left_matcher  = StereoSGBM::create(0,80,wsize);
//            Ptr<StereoSGBM> left_matcher  = StereoSGBM::create(0,max_disp,wsize);
            left_matcher->setP1(24*wsize*wsize);
            left_matcher->setP2(96*wsize*wsize);
            left_matcher->setPreFilterCap(63);
            left_matcher->setMode(StereoSGBM::MODE_HH4);
	    left_matcher->setMinDisparity(-16);
	    left_matcher->setNumDisparities(80);
            wls_filter = createDisparityWLSFilter(left_matcher);
            Ptr<StereoMatcher> right_matcher = createRightMatcher(left_matcher);
...
Rect computeROI(Size2i src_sz, Ptr<StereoMatcher> matcher_instance)
{
    ...
    cout << "We are at line 346 in Disparity_computation.cpp!" << minD << " " << maxD << endl;	
    int xmin = maxD + bs2;
